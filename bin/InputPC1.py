    ...
    try:
        ...
        while True:
            ...
            if state == 0 and command.lower() == "start":
                ...
                img_angle = []
                for _ in range(3):
                    angle = type_angle[int.from_bytes(myserial.read(), "big")]
            ...
            elif state == 1 and command.lower() in [t.lower() for t in img_angle]:
                for key, value in type_img.items():
                    if value.lower() == command.lower():
                        myserial.write(ord(key).to_bytes(1, "big"))
                        for c in range(16):
                            posx = ((0x1F & int.from_bytes(myserial.read(), "big") | (
                                int.from_bytes(myserial.read(), "big") & 0xF) << 5))
                            posy = ((0x1F & int.from_bytes(myserial.read(), "big") | (
                                int.from_bytes(myserial.read(), "big") & 0xF) << 5))
                            mypixelValue = int.from_bytes(
                                myserial.read(), "big")
                            print("pixel x = ", posx, "pixel y = ",
                                  posy, "pixel value =", mypixelValue)
                state = 0
            else:
                print("Command is not correct re-Enter for Active")

    except serial.SerialException:
        print("Port Unusable", file=sys.__stderr__)
